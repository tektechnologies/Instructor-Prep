...
``` 
- I can use this variable here, because the bracket notation expects a string and that is what we get from the let keyIWantToLookUp, when saved in a variable some where. 
...
## Adding new props
- When adding NEW property methods, we will use the `=` instead of the `.` because we are assigning a value to the object.
